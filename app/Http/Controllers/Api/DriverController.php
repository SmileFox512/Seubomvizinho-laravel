...
class DriverController extends Controller
{
    ...
    public function reject(Request $request)
    {
        ...
        // vendor
        $userid = DB::table('users')->where('vendor', $order->restaurant)->get()->first();
        if ($userid != null) {
            $myRequest = new \Illuminate\Http\Request();
            $myRequest->setMethod('POST');
            $myRequest->request->add(['user' => $userid->id]);
            $myRequest->request->add(['title' => Lang::get(470)]); // 'Order status changed',
            $myRequest->request->add(['text' => Lang::get(644) . $id . Lang::get(476) . Lang::get(643)]);  // "Order #",  ' was ' rejected by driver
            $defaultImage = DB::table('settings')->where('param', '=', "notify_image")->get()->first()->value;
            $myRequest->request->add(['imageid' => $defaultImage]);
            MessagingController::sendNotify($myRequest);
        }
    ...
    public function accept(Request $request)
    {
        ...
        if ($order != null) {
            ...
            $myRequest->request->add(['user' => $order->user]);
            $myRequest->request->add(['title' => Lang::get(470)]); // 'Order status changed',
            $myRequest->request->add(['text' => Lang::get(475) . $id . Lang::get(476) . Lang::get(645)]);  // "You order #",  ' was '  accepted by driver
            $myRequest->request->add(['imageid' => $defaultImage]);
            MessagingController::sendNotify($myRequest);
        }
        // vendor
